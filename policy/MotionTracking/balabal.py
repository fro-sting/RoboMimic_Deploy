...
class MotionTracking(FSMState):
    ...
    def __init__(self, state_cmd:StateAndCmd, policy_output:PolicyOutput):
        ...
        with open(config_path, "r") as f:
            ...
            #    这是 `action_to_dof29_index` 的正确生成方式
    ...
    def set_mujoco_data(self, mj_data, mj_model):
        """设置MuJoCo数据引用，用于获取body位置；并打印校验 keypoint 映射"""
        ...
        # 解析并校验 KEYPOINT_BODY_NAMES -> 模型 body id 的映射
        self.keypoint_body_ids = []
        print("[DEBUG] KEYPOINT_BODY_NAMES -> resolved body ids and actual model names:")
        for i, name in enumerate(self.KEYPOINT_BODY_NAMES):
            # 使用 mj_name2id 更通用
            body_id = mujoco.mj_name2id(self.mujoco_model, mujoco.mjtObj.mjOBJ_BODY, name)
            if body_id == -1:
                print(f"  [{i}] '{name}' -> NOT FOUND (id=-1)")
                self.keypoint_body_ids.append(-1)
            else:
                # 从模型通过 id 取回 name 做二次校验（有时 binding 支持 model.body(id).name）
                try:
                    model_name = mj_model.body(body_id).name
                except Exception:
                    model_name = None
                print(f"  [{i}] '{name}' -> id: {body_id}, model.body(id).name: {model_name}")
                self.keypoint_body_ids.append(body_id)

        self.keypoint_body_ids = np.array(self.keypoint_body_ids, dtype=np.int32)
        print(f"[DEBUG] Keypoint body IDs (MuJoCo): {self.keypoint_body_ids}\n")
